...
class Attention(Layer):
    ...
    def call(self, x):
        e = K.tanh(K.dot(x, self.W) + self.b)  # e: (batch_size, time_steps, 1)
        a = K.softmax(e, axis=1)               # a: attention weights
        output = x * a                         # 加权
...
def create_sequences(data, window_size):
    ...
    for i in range(len(data) - window_size):
        ...
        y.append(data[i+window_size][0])  # Close price作为y
...
def make_multistep_sequences(data, window_size=30, future_steps=5):
    ...
    for i in range(len(data) - window_size - future_steps + 1):
        X.append(data[i : i + window_size, :])
        y.append(data[i + window_size : i + window_size + future_steps, 0])  # 只预测 Close
    return np.array(X), np.array(y)
...
def train_and_save_model_with_attention(X_train, y_train, 
    ...
    if layers == 1:
        # 只有一层 LSTM，要用于 Attention，所以 return_sequences=True
        model.add(LSTM(units, return_sequences=True, input_shape=(window_size, X_train.shape[2])))
        if dropout_rate > 0:
            model.add(Dropout(dropout_rate))

    else:
        # 第一层：return_sequences=True
        model.add(LSTM(units, return_sequences=True, input_shape=(window_size, X_train.shape[2])))
        if dropout_rate > 0:
            model.add(Dropout(dropout_rate))

        # 中间层（全部 return_sequences=True）
        for i in range(1, layers - 1):
            model.add(LSTM(units, return_sequences=True))
            if dropout_rate > 0:
                model.add(Dropout(dropout_rate))

        # 最后一层，Attention 接收序列 → 仍然需要 return_sequences=True
        model.add(LSTM(units, return_sequences=True))
        if dropout_rate > 0:
            model.add(Dropout(dropout_rate))
...
def evaluate_model(model_path, history_path, X_test, y_test, scaler=None, title=None, attention=False):
    ...
    if y_test.ndim == 1:
        y_test = y_test[:, None]          # (N,1)
    if y_pred.ndim == 1:
        y_pred = y_pred[:, None]          # (N,1)
...
def evaluate_model_multistep(model_path,
    ...
    if attention:
        ...
        model  = load_model(model_path)
    y_pred  = model.predict(X_test)                 # (samples, future_steps)
    ...
    if scaler is not None:
        ...
        # --- 关键对齐：保证都是 2D (samples, future_steps) ---
